...
export default function GameDetail({ gameId }: GameDetailProps) {
  ...
  const startSession = async (isTrial: boolean) => {
    ...
    const { data, error } = await cloud.startGameSession(gameId, isTrial);
    if (!error && data) {
      setActiveSession(data);
      if (isTrial) setTrialSession(data);
      navigate('cloud-player', data.id);
  ...
  const endSession = async () => {
    ...
    await cloud.stopGameSession(activeSession.id, 'ended');
  ...
  const handleTrialExpire = async () => {
    ...
    if (activeSession) {
      await cloud.stopGameSession(activeSession.id, 'expired');
      setActiveSession(null);
    }
  };
